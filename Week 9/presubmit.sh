...
if [ "$1" = "" ]
then
    echo "You forgot to add the assignment name, e.g. 'ProblemSheet1'."
...
ghc -package mtl -package random $1.hs -odir $temp_dir -hidir $temp_dir
...
then
    ...
    echo -e "\t(i) your file is called something like 'ProblemSheetX.hs'"
    echo -e "\t(ii) you did not change the top of the template"
    echo "and try again."
    exit 1
fi

ghc -package mtl -package random -XSafe $1.hs -odir $temp_dir -hidir $temp_dir
